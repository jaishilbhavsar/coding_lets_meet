...
var post = {
    ...
    addPost: function (ps, callback) {
        return db.query("insert into post_tbl values(?,?,?,?,?,?,?)", [null, ps.post_title, ps.post_des, ps.post_pic, ps.post_date, ps.fk_user_id, ps.fk_comm_id], callback);
    },
    ...
    getPostAndUser: function (id, callback) {
        return db.query("select p.*,u.* from post_tbl p,user_tbl u where p.fk_user_id=u.user_id and p.post_id=?", [id],callback);
    }
};
